...
SegmentConnector:function (state, sourceScaled, targetScaled, d, result) {
  ...
  targetScaled = mxEdgeStyle.scaleCellState(targetScaled, state.view.scale);
  var lastPushed = 0 < result.length ? result[0] : null, l = !0, m = null, pt = pts[0];
  null == pt && null != sourceScaled ? pt = new mxPoint(state.view.getRoutingCenterX(sourceScaled), state.view.getRoutingCenterY(sourceScaled)) : null != pt && (pt = pt.clone());
  ...
  if (null != d && 0 < d.length) {
    ...
    l && (null != pts[0] && pts[0].y != m.y || null ==
      pts[0] && null != sourceScaled && (m.y < sourceScaled.y || m.y > sourceScaled.y + sourceScaled.height)) ? pushPoint(new mxPoint(pt.x, m.y)) : !l && (null != pts[0] && pts[0].x != m.x || null == pts[0] && null != sourceScaled && (m.x < sourceScaled.x || m.x > sourceScaled.x + sourceScaled.width)) && pushPoint(new mxPoint(m.x, pt.y));
    l ? pt.y = m.y : pt.x = m.x;
    for (r = 0; r < q.length; r++) l = !l, m = q[r], l ? pt.y = m.y : pt.x = m.x, pushPoint(pt.clone())
  } else m = pt, l = !0;
  pt = pts[lastInx];
  null == pt && null != targetScaled && (pt = new mxPoint(state.view.getRoutingCenterX(targetScaled), state.view.getRoutingCenterY(targetScaled)));
  null != pt && null != m && (l && (null != pts[lastInx] && pts[lastInx].y != m.y || null == pts[lastInx] && null != targetScaled && (m.y < targetScaled.y || m.y > targetScaled.y + targetScaled.height)) ? pushPoint(new mxPoint(pt.x, m.y)) : !l && (null !=
    pts[lastInx] && pts[lastInx].x != m.x || null == pts[lastInx] && null != targetScaled && (m.x < targetScaled.x || m.x > targetScaled.x + targetScaled.width)) && pushPoint(new mxPoint(m.x, pt.y)));
