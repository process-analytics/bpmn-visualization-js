...
SegmentConnector:function (state, b, c, d, e) {
  function pushPoint(b) {
    ...
    if (null == k || 1 <= Math.abs(k.x - b.x) || Math.abs(k.y - b.y) >= Math.max(1, state.view.scale)) e.push(b), k = b;
    return k
  }
  ...
  if (null != d && 0 < d.length) {
    ...
    l && (null != g[0] && g[0].y != m.y || null ==
      g[0] && null != b && (m.y < b.y || m.y > b.y + b.height)) ? pushPoint(new mxPoint(n.x, m.y)) : !l && (null != g[0] && g[0].x != m.x || null == g[0] && null != b && (m.x < b.x || m.x > b.x + b.width)) && pushPoint(new mxPoint(m.x, n.y));
    l ? n.y = m.y : n.x = m.x;
  ...
  null != n && null != m && (l && (null != g[p] && g[p].y != m.y || null == g[p] && null != c && (m.y < c.y || m.y > c.y + c.height)) ? pushPoint(new mxPoint(n.x, m.y)) : !l && (null !=
    g[p] && g[p].x != m.x || null == g[p] && null != c && (m.x < c.x || m.x > c.x + c.width)) && pushPoint(new mxPoint(m.x, n.y)));
