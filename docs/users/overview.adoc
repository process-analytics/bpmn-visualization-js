...
It is possible to change the default them by configuring CSS in page holding the diagram or extending `bpmn-visualization`.

* Extending gives the full control on the theme customization, including colors, size, icons, and more.
* CSS is the fastest to implement but provides less feature. For instance, you cannot change the icons or precisely control the size of the strokes.
