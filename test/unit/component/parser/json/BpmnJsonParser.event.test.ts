...
import type { ExpectedEventShape, ExpectedBoundaryEventShape } from '../../../helpers/bpmn-model-expect';
import { verifyShape } from '../../../helpers/bpmn-model-expect';
import type { BuildDefinitionParameter, BuildEventDefinitionParameter, BuildEventParameter } from '../../../helpers/JsonBuilder';
...
function testMustConvertShapes(eventParameter: BuildEventParameter | BuildEventParameter[], omitExpectedShape: OmitExpectedEventShape, processIsArray = false): void {
  ...
  const model = parseJsonAndExpectEvent(json, omitExpectedShape.eventDefinitionKind, Array.isArray(eventParameter) ? eventParameter.length : 1);
